...
static struct intel_clk_branch grx500_branch_clks[] __initdata = {
	...
	/*
	 * The boot chain leaves this SoC's peripheral gates in a working
	 * state and the vendor driver never changes one: its "initial gate
	 * default" write is inert (the mask is passed as the shift, with a
	 * zero mask), and every vendor and reference board instead boots with
	 * clk_ignore_unused on the command line. Registering the gates
	 * CLK_IGNORE_UNUSED is the same policy expressed in the driver, where
	 * it does not depend on a command line the platform cannot control.
	 * The gates the vendor declares as must-be-on are CBM, EBU, UART,
	...
	INTEL_GATE(GCLK_USB1, "g_usb1", NULL, CLK_IGNORE_UNUSED, GRX500_CLK_GSR1,
		   G_USB1_SHIFT, GATE_CLK_HW, 0),
	INTEL_GATE(GCLK_SSO, "g_sso", NULL, CLK_IGNORE_UNUSED, GRX500_CLK_GSR1,
		   G_SSO_SHIFT, GATE_CLK_HW, 0),
	INTEL_GATE(GCLK_GPTC0, "g_gptc0", NULL, CLK_IGNORE_UNUSED, GRX500_CLK_GSR1,
		   G_GPTC0_SHIFT, GATE_CLK_HW, 0),
	INTEL_GATE(GCLK_GPTC1, "g_gptc1", NULL, CLK_IGNORE_UNUSED, GRX500_CLK_GSR1,
		   G_GPTC1_SHIFT, GATE_CLK_HW, 0),
	INTEL_GATE(GCLK_GPTC2, "g_gptc2", NULL, CLK_IGNORE_UNUSED, GRX500_CLK_GSR1,
		   G_GPTC2_SHIFT, GATE_CLK_HW, 0),
	INTEL_GATE(GCLK_UART, "g_uart", NULL, CLK_IGNORE_UNUSED, GRX500_CLK_GSR1,
		   G_UART_SHIFT, GATE_CLK_HW, 0),
	INTEL_GATE(GCLK_PCIE0, "g_pcie0", NULL, CLK_IGNORE_UNUSED, GRX500_CLK_GSR2,
		   G_PCIE0_SHIFT, GATE_CLK_HW, 0),
	INTEL_GATE(GCLK_PCIE1, "g_pcie1", NULL, CLK_IGNORE_UNUSED, GRX500_CLK_GSR2,
		   G_PCIE1_SHIFT, GATE_CLK_HW, 0),
	INTEL_GATE(GCLK_PCIE2, "g_pcie2", NULL, CLK_IGNORE_UNUSED, GRX500_CLK_GSR2,
		   G_PCIE2_SHIFT, GATE_CLK_HW, 0),
	INTEL_GATE(GCLK_I2C, "g_i2c", NULL, CLK_IGNORE_UNUSED, 0, 0, GATE_CLK_VT, 0),
