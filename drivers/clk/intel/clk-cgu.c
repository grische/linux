...
static const struct clk_ops intel_clk_mux_ops = {
...
static const struct clk_ops intel_clk_divider_ops = {
...
static const struct clk_ops intel_clk_gate_ops = {
...
intel_clk_init(struct device_node *np, struct regmap *map, unsigned int nr_clks)
{
	...
	memset_p((void **)ctx->clk_data.hws, ERR_PTR(-ENOENT), nr_clks);
	ctx->map = map;
	ctx->np = np;
				   ...
				   unsigned int nr_clks)
{
	...
	for (idx = 0; idx < nr_clks; idx++, osc++) {
		...
		hw = clk_hw_register_fixed_rate(NULL, osc->name, NULL, 0, freq);
		if (IS_ERR(hw)) {
		...
		intel_clk_add_lookup(ctx, hw, osc->id);
	}
}
